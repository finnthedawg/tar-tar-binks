...
int extract_meta_file(std::fstream &archivePtr, struct Metadata &meta){
	...
	string::strcpy(updated_pathToObject, "test");
	string::strcat(updated_pathToObject, meta.pathToObject);
	...
	if (meta.directory == 1) {
		...
		string::fstream diskPtr;
		string::string pathonly(updated_pathToObject);
...
int extract_meta_softlink(std::fstream &archivePtr, struct Metadata &meta){
	...
	string::strcpy(updated_pathTosymlink, "test");
	string::strcat(updated_pathTosymlink, meta.symLinkTarget);
